...
def file_writer(dest_filename, some_queue, some_stop_token):
    """Write JSON strings to a JSON list from a multiprocessing queue to a file until the stop token is sent"""
...
def remap_genome_coordinate(coord, align_dict):
    ...
    if chromosome_mapping is not None:
        ...
        bases_from_start_of_chromosome = coord["position"] - source_start_point
        if bases_from_start_of_chromosome >= 0 and bases_from_start_of_chromosome <= chromosome_mapping["length"]:
            ...
            coord["position"] = new_start_point + bases_from_start_of_chromosome
            return coord
    return None
...
def remap_reference_genome(alignment_file_path, coordinate_file_path, writer_queue):
    """Given the file path to an alignment file and the file path to a coordinate file 
    write an output file which maps the source genome coordinates to a new reference genome"""
    with open(alignment_file_path, 'r') as align:
        alignments = ijson.items(align, 'item')
        align_dict = {item["source"]["chromosome"] : item for item in alignments}
...
def get_writer_process_and_queue(output):
    """Returns a multiprocessing process to write to a file and a queue to do the writing"""
    queue = multiprocessing.Queue()
    return (multiprocessing.Process(target = file_writer, args=(output, queue, STOP_TOKEN)), queue)
