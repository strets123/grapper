...
class TestGrapper(unittest.TestCase):
    ...
    def test_remap_genome_coordinate(self):
        ...
        self.assertEqual(new_mapping, { "chromosome": "2", "position": 350, "reference": "A" })
    ...
    def test_handle_command(self):
        """Given the JSON alignment file in tests/test_data/alignment.json with contents

        [{ "length": 100, "source": { "chromosome": "1", "start": 100 }, "target": { "chromosome": "2", "start": 

        300 } },

        { "length": 200, "source": { "chromosome": "2", "start": 300 }, "target": { "chromosome": "7", "start": 

        20 } }

        ]

        And the JSON source coordinate file in tests/test_data/source_coordinates.json with contents
        ...
        with open(OUTPUT_FILE, 'r') as output:
            ...
            self.assertEqual(dict_list, [{ "chromosome": "2", "position": 350, "reference": "A" },
                                            { "chromosome": "7", "position": 20, "reference": "C" }])
    ...
    def test_handle_outside_range_or_unmapped(self):
        ...
        And the JSON source coordinate file in tests/test_data/source_coordinates_with_invalid.json with contents
